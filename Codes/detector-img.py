...
if __name__ == "__main__":
    ...
    parser.add_argument('image_path', help="Video file",default="../Data/espacio-libre.jpg")
    parser.add_argument('regions_path', help="Regions file",
                        default="../Results/regions.p")
    ...
    VIDEO_SOURCE = args.image_path
    alpha = 0.6
    print(VIDEO_SOURCE)
    img_capture = cv2.imread(VIDEO_SOURCE)
    fig = plt.figure()
    plt.imshow(img_capture)
    plt.show()
    overlay = img_capture.copy()
    rgb_image = img_capture[:, :, ::-1]
    ...
    for parking_area, overlap_areas in zip(parked_car_boxes, overlaps):
        ...
        if max_IoU_overlap < 0.15:
            ...
            free_space = True
    cv2.addWeighted(overlay, alpha, img_capture, 1 - alpha, 0, img_capture)

    # cv2.imshow('output', img_capture)    
    # cv2.waitKey(0)
    cv2.imwrite('../Results/output123.jpg', img_capture)
 
    print("output saved as out")
